...
class NodeBloomTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Verify mininodes are connected to resistanced nodes
        ...
        # Mininodes send filterclear message to resistanced node.
        nobf_node.send_message(msg_filterclear())
        bf_node.send_message(msg_filterclear())

        time.sleep(3)

        # Verify mininodes are still connected to resistanced nodes
        peerinfo = self.nodes[0].getpeerinfo()
        versions = [x["version"] for x in peerinfo]
        assert_equal(1, versions.count(SAPLING_PROTO_VERSION))
        peerinfo = self.nodes[1].getpeerinfo()
        ...
        # Mininodes send filteradd message to resistanced node.
