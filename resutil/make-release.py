...
def main(args=sys.argv[1:]):
    """
    Perform the final Resistance release process up to the git tag.
    """
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "resistance-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "resistance-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './resistance-make-release.log'
    ...
    logging.info('resistance make-release.py debug log: %r', logname)
