...
#define MIND 1000
const static uint32_t MINDENOMS[] = { MIND, MIND, 100*MIND, MIND, MIND, MIND, MIND, MIND, // major currencies
...
static bits256 bits256_doublesha256(char *deprecated,uint8_t *data,int32_t datalen)
{
 ...
 */
static int rmd160_vinit(struct rmd160_vstate * md)
{
...
/**
 ...
 @return 0 if successful
 */
static HASH_PROCESS(rmd160_vprocess, rmd160_vcompress, rmd160, 64)
...
/**
 ...
 @return 0 if successful
 */
static int rmd160_vdone(struct rmd160_vstate * md, unsigned char *out)
{
    int i;
    if (md->curlen >= sizeof(md->buf)) {
        return -1;
...
static void calc_rmd160(char deprecated[41],uint8_t buf[20],uint8_t *msg,int32_t len)
{
...
static void calc_rmd160_sha256(uint8_t rmd160[20],uint8_t *data,int32_t datalen)
{
    bits256 hash;
    vcalc_sha256(0,hash.bytes,data,datalen);
...
static int32_t bitcoin_addr2rmd160(uint8_t *addrtypep,uint8_t rmd160[20],char *coinaddr)
{
...
static char *bitcoin_address(char *coinaddr,uint8_t addrtype,uint8_t *pubkey_or_rmd160,int32_t len)
{
...
static int32_t bitweight(uint64_t x)
{
    int i,wt = 0;
    for (i=0; i<64; i++)
        if ( (1LL << i) & x )
...
static int32_t is_hexstr(char *str,int32_t n)
{
    int32_t i;
    if ( str == 0 || str[0] == 0 )
        return(0);
...
static unsigned char _decode_hex(char *hex) { return((unhex(hex[0])<<4) | unhex(hex[1])); }
...
static int32_t init_hexbytes_noT(char *hexbytes,unsigned char *message,long len)
{
    int32_t i;
    if ( len <= 0 )
    {
...
static char *bits256_str(char hexstr[65],bits256 x)
{
...
static int32_t iguana_rwbignum(int32_t rwflag,uint8_t *serialized,int32_t len,uint8_t *endianedp)
{
    int32_t i;
    if ( rwflag == 0 )
    {
...
static char *parse_conf_line(char *line,char *field)
{
    line += strlen(field);
...
static void queue_enqueue(char *name,queue_t *queue,struct queueitem *item)
{
...
static struct queueitem *queue_dequeue(queue_t *queue)
{
...
static void *queue_delete(queue_t *queue,struct queueitem *copy,int32_t copysize)
{
    struct queueitem *item = 0;
    lock_queue(queue);
    if ( queue->list != 0 )
...
static void *queue_clone(queue_t *clone,queue_t *queue,int32_t size)
{
...
static int32_t queue_size(queue_t *queue)
{
    int32_t count = 0;
...
static void iguana_initQ(queue_t *Q,char *name)
{
...
static void komodo_nameset(char *symbol,char *dest,char *source)
{
    if ( source[0] == 0 )
    {
        strcpy(symbol,(char *)"RES");
        strcpy(dest,(char *)"BTC");
    }
    else
    {
        strcpy(symbol,source);
        strcpy(dest,(char *)"RES");
    }
}
...
static size_t accumulatebytes(void *ptr,size_t size,size_t nmemb,struct return_string *s)
{
...
static void init_string(struct return_string *s)
{
    s->len = 0;
...
static char *bitcoind_RPC(char **retstrp,char *debugstr,char *url,char *userpass,char *command,char *params)
{
