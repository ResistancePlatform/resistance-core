...
#if defined(NDEBUG)
# error "Resistance cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Resistance Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("resistance-scriptch");
    ...
    const CChainParams& chainparams, CBlockIndex * const pindexPrev)
{
    ...
    // In Resistance this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see Resistance protocol spec
    ...
    if (nHeight >= chainparams.GetConsensus().nSubsidySlowStartHeight && nHeight <= consensusParams.GetLastPorRewardBlockHeight()) {
        ...
        BOOST_FOREACH(const CTxOut& output, block.vtx[0].vout) {
            if (output.scriptPubKey == chainparams.GetPorRewardScriptAtHeight(nHeight)) {
    ...
    if (nHeight >= chainparams.GetConsensus().nSubsidySlowStartHeight && nHeight <= consensusParams.GetLastPlatformDevFundBlockHeight()) {
        ...
        BOOST_FOREACH(const CTxOut& output, block.vtx[0].vout) {
            if (output.scriptPubKey == chainparams.GetPlatformDevFundScriptAtHeight(nHeight)) {
