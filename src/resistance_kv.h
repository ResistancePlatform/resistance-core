...
#ifndef resistance_kv__h
#define resistance_kv__h

#include "resistance_curve25519.h"

static int32_t resistance_kvnumdays(uint32_t flags)
{
    int32_t numdays;
...
static int32_t resistance_kvduration(uint32_t flags)
{
    return(resistance_kvnumdays(flags) * RESISTANCE_KVDURATION);
}

static uint64_t resistance_kvfee(uint32_t flags,int32_t opretlen,int32_t keylen)
{
    int32_t numdays,k; uint64_t fee;
    if ( (k= keylen) > 32 )
        k = 32;
    numdays = resistance_kvnumdays(flags);
    if ( (fee= (numdays*(opretlen * opretlen / k))) < 100000 )
        fee = 100000;
    return(fee);
}

static int32_t resistance_kvsearch(uint256 *pubkeyp,int32_t current_height,uint32_t *flagsp,int32_t *heightp,uint8_t value[IGUANA_MAXSCRIPTSIZE],uint8_t *key,int32_t keylen)
{
    struct resistance_kv *ptr; int32_t duration,retval = -1;
    *heightp = -1;
    *flagsp = 0;
    memset(pubkeyp,0,sizeof(*pubkeyp));
    portable_mutex_lock(&RESISTANCE_KV_mutex);
    HASH_FIND(hh,RESISTANCE_KV,key,keylen,ptr);
    if ( ptr != 0 )
    {
        duration = resistance_kvduration(ptr->flags);
        ...
        {
            HASH_DELETE(hh,RESISTANCE_KV,ptr);
            if ( ptr->value != 0 )
                free(ptr->value);
            if ( ptr->key != 0 )
                free(ptr->key);
    ...
    portable_mutex_unlock(&RESISTANCE_KV_mutex);
    if ( retval < 0 )
    {
        // search rawmempool
    }
...
static void resistance_kvupdate(uint8_t *opretbuf,int32_t opretlen,uint64_t value)
{
    static uint256 zeroes;
    uint32_t flags; uint256 pubkey,refpubkey,sig; int32_t i,refvaluesize,hassig,coresize,haspubkey,height,kvheight; uint16_t keylen,valuesize,newflag = 0; uint8_t *key,*valueptr,keyvalue[IGUANA_MAXSCRIPTSIZE]; struct resistance_kv *ptr; char *transferpubstr,*tstr; uint64_t fee;
    ...
    if ( keylen+13 > opretlen )
    {
        ...
        if ( ++counter < 1 )
            LogPrintf("resistance_kvupdate: keylen.%d + 13 > opretlen.%d, this can be ignored\n",keylen,opretlen);
        return;
    }
    valueptr = &key[keylen];
    fee = resistance_kvfee(flags,opretlen,keylen);
    ...
    if ( value >= fee )
    {
        ...
        if ( opretlen == coresize || opretlen == coresize+sizeof(uint256) || opretlen == coresize+2*sizeof(uint256) )
        {
            ...
            if ( (refvaluesize= resistance_kvsearch((uint256 *)&refpubkey,height,&flags,&kvheight,&keyvalue[keylen],key,keylen)) >= 0 )
            {
                if ( memcmp(&zeroes,&refpubkey,sizeof(refpubkey)) != 0 )
                {
                    if ( resistance_kvsigverify(keyvalue,keylen+refvaluesize,refpubkey,sig) < 0 )
                    {
                        LogPrintf("resistance_kvsigverify error [%d]\n",coresize-13);
                        return;
                    }
                }
            }
            ...
            if ( ptr != 0 )
            {
                //if ( (ptr->flags & RESISTANCE_KVPROTECTED) != 0 )
                {
                    tstr = (char *)"transfer:";
            ...
            {
                ptr = (struct resistance_kv *)calloc(1,sizeof(*ptr));
                ptr->key = (uint8_t *)calloc(1,keylen);
                ptr->keylen = keylen;
                memcpy(ptr->key,key,keylen);
                newflag = 1;
                HASH_ADD_KEYPTR(hh,RESISTANCE_KV,ptr->key,ptr->keylen,ptr);
                //LogPrintf("KV add.(%s) (%s)\n",ptr->key,valueptr);
            }
            if ( newflag != 0 || (ptr->flags & RESISTANCE_KVPROTECTED) == 0 )
            {
                if ( ptr->value != 0 )
