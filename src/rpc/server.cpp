...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Resistance server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "Resistance server stopping";
}
...
{
    return "> resistance-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:8132/\n";
}

string experimentalDisabledHelpMsg(const string& rpc, const string& enableArg)
{
    return "\nWARNING: " + rpc + " is disabled.\n"
        "To enable it, restart resistanced with the -experimentalfeatures and\n"
        "-" + enableArg + " commandline options, or add these two lines\n"
        "to the resistance.conf file:\n\n"
